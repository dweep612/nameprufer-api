...
exports.youtubeCheck = (req, res) => {
  ...
  if (
    youtubeRegex.test(req.params.username) &&
    req.params.username.length <= 15
  ) {
    fetch("https://www.youtube.com/" + req.params.username)
      .then((response) => {
        if (response.status === 404) {
          return res.json({ success: "Username Available" });
        } else {
...
exports.redditCheck = (req, res) => {
  var redditRegex = /^[a-zA-Z0-9]*$\w{0,38}/g;

  if (
    redditRegex.test(req.params.username) &&
    req.params.username.length <= 15
  ) {
    fetch("https://www.reddit.com/user/" + req.params.username)
      .then((response) => {
        if (response.status === 404) {
          return res.json({ success: "Username Available" });
        } else {
